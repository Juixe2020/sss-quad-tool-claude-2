...
.live-match-card{background:var(--bg-sec);border:1px solid var(--border);border-radius:10px;padding:0.9rem 1.2rem;margin-bottom:0.5rem;cursor:pointer;transition:border-color 0.15s;}
.live-match-card:hover{border-color:var(--accent);}
.live-match-players{font-family:'Space Mono',monospace;font-size:0.9rem;font-weight:700;color:var(--text);}
.live-match-meta{font-size:0.75rem;color:var(--muted);margin-top:3px;}
.live-badge-live{color:#EF4444;font-family:monospace;font-size:0.7rem;font-weight:700;letter-spacing:1px;}
.live-badge-upcoming{color:#F59E0B;font-family:monospace;font-size:0.7rem;font-weight:700;letter-spacing:1px;}
...
def get_live_matches():
    ...
    if api:
        return api
    # Fallback — seeded from the live feed fetched earlier in this session
    return [
        ...
        {"p1":"Cash J / Schuurs D",     "p2":"Vavassori A / Errani S",  "tournament":"ATP Indian Wells Mixed Doubles",      "round":"—",           "surface":"Hard (Outdoor)","status":"scheduled"},
    ]
...
MATCHES = {
    "🎾 Draper [15] vs. Medvedev [4] — ATP Indian Wells QF": {
        ...
        "q2":{"title":"Environmental Context","icon":"🌵","body":"IW night sessions: <strong>heavy, dense air</strong> slows the ball. Altitude ~480m reduces serve dominance. Wind: calm. IW plays ~6% slower than tour average."},
        "q3":{"title":"Psychological / Narrative","icon":"🧠","body":"Draper riding <strong>alpha momentum</strong> after beating Djokovic. Defending his 2025 title. Medvedev faded mid-match vs Fils in R16."},
...
def render_players(p1, p1_seed, p1_odds, p1_form, p2, p2_seed, p2_odds, p2_form):
    ...
    with ca: st.markdown("**" + p1 + "** &nbsp; `" + p1_seed + "`", unsafe_allow_html=True)
    with cb: st.markdown("Form: " + form_badges(p1_form), unsafe_allow_html=True)
    with cc: st.markdown('<span style="background:#0d9268;color:#fff;padding:3px 10px;border-radius:6px;font-family:monospace;font-weight:700;">' + p1_odds + '</span>', unsafe_allow_html=True)
    ...
    with ca2: st.markdown("**" + p2 + "** &nbsp; `" + p2_seed + "`", unsafe_allow_html=True)
    with cb2: st.markdown("Form: " + form_badges(p2_form), unsafe_allow_html=True)
    with cc2: st.markdown('<span style="background:#0d9268;color:#fff;padding:3px 10px;border-radius:6px;font-family:monospace;font-weight:700;">' + p2_odds + '</span>', unsafe_allow_html=True)
...
with tab_ai:
    ...
    input_mode = st.radio(
        "Input mode",
        ["🔴  Live & Upcoming Matches", "✏️  Manual Entry"],
        horizontal=True,
    ...
    if input_mode == "🔴  Live & Upcoming Matches":
        ...
        if not live_matches:
            ...
            for m in live_matches:
                ...
                labels.append(badge + "  " + m["p1"] + " vs " + m["p2"] + "  ·  " + m["tournament"])
        ...
        st.markdown('<div class="form-section-title">➕ Add Odds & Context</div>', unsafe_allow_html=True)
        st.markdown('<p style="color:#94A3B8;font-size:0.82rem;margin-bottom:1rem;">Player names, tournament and surface are pre-filled. Add odds, form and any extra context before generating.</p>', unsafe_allow_html=True)
    ...
    with fc1:
        p1_name     = st.text_input("Player 1 Name",       value=pre.get("p1_name",""),    placeholder="e.g. Jack Draper",              key="p1n")
        p1_seed     = st.text_input("P1 Seed / Ranking",   value="",                        placeholder="e.g. #15",                      key="p1s")
        p1_odds     = st.text_input("P1 Odds",             value="",                        placeholder="e.g. +145",                     key="p1o")
        p1_form     = st.text_input("P1 Last 5 Results",   value="",                        placeholder="W W L W W",                     key="p1f")
        p1_strength = st.text_input("P1 Key Strength",     value="",                        placeholder="e.g. Tiebreak dominance",       key="p1st")
    with fc2:
        p2_name     = st.text_input("Player 2 Name",       value=pre.get("p2_name",""),    placeholder="e.g. Daniil Medvedev",          key="p2n")
        p2_seed     = st.text_input("P2 Seed / Ranking",   value="",                        placeholder="e.g. #4",                       key="p2s")
        p2_odds     = st.text_input("P2 Odds",             value="",                        placeholder="e.g. -175",                     key="p2o")
        p2_form     = st.text_input("P2 Last 5 Results",   value="",                        placeholder="W W W L W",                     key="p2f")
        p2_strength = st.text_input("P2 Key Strength",     value="",                        placeholder="e.g. Baseline consistency",     key="p2st")

    st.markdown('<div class="form-section-title">🏟️ Match Context</div>', unsafe_allow_html=True)
    mc1, mc2, mc3 = st.columns(3)

    surface_options     = ["Hard (Outdoor)","Hard (Indoor)","Clay","Grass"]
    surface_default_idx = surface_options.index(pre["surface"]) if pre.get("surface") in surface_options else 0

    with mc1:
        tournament = st.text_input("Tournament", value=pre.get("tournament",""), placeholder="e.g. ATP Indian Wells", key="tourn")
        round_     = st.text_input("Round",      value=pre.get("round",""),      placeholder="e.g. Quarterfinal",     key="rnd")
    with mc2:
        surface     = st.selectbox("Surface",    surface_options, index=surface_default_idx, key="surf")
    ...
    conditions = st.text_input("Conditions / Extra Notes", value="", placeholder="e.g. Night session, heavy air, defending champion...", key="cond")
    ...
    if "ai_result" in st.session_state and "ai_form" in st.session_state:
        ...
        with lc3:
            log_notes = st.text_input("Notes (optional)", placeholder="e.g. High confidence", key="log_notes")
...
with tab_log:
    ...
    if df.empty:
        ...
        st.markdown('<p style="color:#94A3B8;font-size:0.82rem;margin-bottom:0.8rem;">Edit the <strong>Result</strong> column (WIN / LOSS / PENDING) directly in the table. P&L auto-calculates on save.</p>', unsafe_allow_html=True)
        ...
        # Auto-calc P&L for newly settled rows
